...
def main():
    ...
    parser.add_argument("--run_dir", type=str, default="runs/ddpg")
    ...
    ddpg_cfg = DDPGConfig(**cfg["ddpg"])
    agent = DDPGAgent(obs_dim, action_dim, device, ddpg_cfg,
    ...
    for _ in range(min_init_episodes):
        ep, info = collect_episode(env, policy_fn=lambda ob: np.random.uniform(-1,1,size=(action_dim,)))
    ...
    with open(log_path, "w") as logf:
        while total_steps < args.steps:
            # Rollout with current policy + noise
            def pol(obs):
                a, _ = agent.act(obs)
                return a
            ...
            # Updates
            updates = int(ddpg_cfg.updates_per_step * ep.length())
